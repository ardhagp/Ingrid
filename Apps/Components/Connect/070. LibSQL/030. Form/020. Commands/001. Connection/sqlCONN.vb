...
Namespace Commands.CONN
    ...
    Public Class Editor
        ...
        Public Function PUSHData(ByVal ConnectionName As String, ByVal Address As String, ByVal Port As String, ByVal Username As String, ByVal Password As String, ByVal DataStorage As String, ByVal FileStorage As String, ByVal IsDefault As Boolean, ByVal RowID As String, ByVal IsNew As Boolean, ByVal IsPasswordChange As Boolean)
            Dim _Success As Boolean
            Dim _IsDefaultActivate As String
            ...
            Try
                ...
                If (IsNew) Then
                    V_DBR_SQLITE(1).Query = _IsDefaultActivate & String.Format("insert into serverlist(ID, CONNECTIONNAME, SERVERADDRESS, SERVERPORT, USERNAME, PASSWORD, DBFORDATA, DBFORFILE, DEFAULTCONNECTION) values ('{0}','{1}','{2}',{3},'{4}','{5}','{7}','{8}',{6})", RowID, ConnectionName, Address, Port, Username, CMCv.Security.Encrypt.AES(Password), IsDefault, DataStorage, FileStorage)
                Else
                    ...
                    Else
                        V_DBR_SQLITE(1).Query = _IsDefaultActivate & String.Format("update serverlist set CONNECTIONNAME='{1}', SERVERADDRESS='{2}', SERVERPORT={3}, USERNAME='{4}', PASSWORD='{5}', DEFAULTCONNECTION={6}, DBFORDATA='{7}', DBFORFILE='{8}' where ID='{0}'", RowID, ConnectionName, Address, Port, Username, CMCv.Security.Encrypt.AES(Password), IsDefault, DataStorage, FileStorage)
                    End If
                End If
