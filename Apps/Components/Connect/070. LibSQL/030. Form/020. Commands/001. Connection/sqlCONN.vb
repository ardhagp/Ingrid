...
Namespace Commands.CONN
    ...
    Public Class Editor
        <SupportedOSPlatform("windows")>
        Public Shared Sub GETRowValue(ByVal RowID As String, ByVal ConnectionName As CMCv.txt, ByVal Address As CMCv.txt, ByVal Port As CMCv.txt, ByVal Username As CMCv.txt, ByVal Password As CMCv.txt, ByVal OldPassword As String, ByVal DataStorage As CMCv.txt, ByVal FileStorage As CMCv.txt, ByVal IsDefault As CMCv.chk)
        ...
        <SupportedOSPlatform("windows")>
        Public Shared Function PUSHData(ByVal ConnectionName As String, ByVal Address As String, ByVal Port As String, ByVal Username As String, ByVal Password As String, ByVal DataStorage As String, ByVal FileStorage As String, ByVal IsDefault As Boolean, ByVal RowID As String, ByVal IsNew As Boolean, ByVal IsPasswordChange As Boolean)
            ...
            Try
                ...
                If (IsNew) Then
                    V_DBR_SQLITE(1).Query = V_IsDefaultActivate & $"insert into serverlist(ID, CONNECTIONNAME, SERVERADDRESS, SERVERPORT, USERNAME, PASSWORD, DBFORDATA, DBFORFILE, DEFAULTCONNECTION) values ('{RowID}','{ConnectionName}','{Address}',{Port},'{Username}','{Security.Encrypt.AES(Password)}','{DataStorage}','{FileStorage}',{IsDefault})"
                Else
                    If Not (IsPasswordChange) Then
                        V_DBR_SQLITE(1).Query = V_IsDefaultActivate & String.Format("update serverlist set CONNECTIONNAME='{1}', SERVERADDRESS='{2}', SERVERPORT={3}, USERNAME='{4}', DEFAULTCONNECTION={5}, DBFORDATA='{6}', DBFORFILE='{7}' where ID='{0}'", RowID, ConnectionName, Address, Port, Username, IsDefault, DataStorage, FileStorage)
                    Else
                        V_DBR_SQLITE(1).Query = V_IsDefaultActivate & String.Format("update serverlist set CONNECTIONNAME='{1}', SERVERADDRESS='{2}', SERVERPORT={3}, USERNAME='{4}', PASSWORD='{5}', DEFAULTCONNECTION={6}, DBFORDATA='{7}', DBFORFILE='{8}' where ID='{0}'", RowID, ConnectionName, Address, Port, Username, Security.Encrypt.AES(Password), IsDefault, DataStorage, FileStorage)
                    End If
                End If
